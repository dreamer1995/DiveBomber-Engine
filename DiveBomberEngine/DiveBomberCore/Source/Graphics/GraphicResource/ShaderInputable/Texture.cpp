...
namespace DiveBomber::GraphicResource
{
	...
	void Texture::ApplyConfig()
	{
		...
		{
		case DiveBomber::GraphicResource::Texture::TextureLoadType::TLT_Icon:
			textureParam.generateMip = false;
			textureParam.textureDimension = config["textureDimension"];
			break;
		case DiveBomber::GraphicResource::Texture::TextureLoadType::TLT_DiffuseIrradiance:
			textureParam.generateMip = false;
			textureParam.textureDimension = TextureDimension::TDS_TextureCube;
			break;
		case DiveBomber::GraphicResource::Texture::TextureLoadType::TLT_SpecularMip:
	...
	void Texture::LoadTexture()
	{
		...
		switch (textureLoadType)
		{
		case DiveBomber::GraphicResource::Texture::TextureLoadType::TLT_Standard:
			loadPath = cachePath;
			break;
		case DiveBomber::GraphicResource::Texture::TextureLoadType::TLT_Icon:
	...
	void Texture::GenerateMipMaps(wrl::ComPtr<ID3D12Resource>& uavBuffer)
	{
		...
		std::shared_ptr<Material> material = GlobalResourceManager::Resolve<Material>(EngineMaterialDirectoryW + generateMipName + L"Material", EngineShaderDirectoryW + generateMipName + L".hlsl");
	...
	void Texture::GenerateCubeMap(wrl::ComPtr<ID3D12Resource>& uavBuffer, wrl::ComPtr<ID3D12Resource>& cubeSourceTextureBuffer, bool readSRGB)
	{
		...
		std::shared_ptr<Material> material = GlobalResourceManager::Resolve<Material>(EngineMaterialDirectoryW + generateCubeName + L"Material", EngineShaderDirectoryW + generateCubeName + L".hlsl");
